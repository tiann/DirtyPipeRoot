...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(v -> {
            ...
            boolean expolit = Check.check(tmpfile.getAbsolutePath());

            try {
                String content = new String(Files.readAllBytes(tmpfile.toPath()));
                Log.w(TAG, "content: " + content);
                boolean explotable = content.contains("test");

                Analytics.trackEvent("vunlerable", new HashMap<String, String>() {{
                    put("vunlerable", String.valueOf(expolit));
                }});

                button.setBackgroundColor(explotable ? Color.GREEN : Color.RED);
            } catch (IOException e) {
                Log.e(TAG, "err", e);
            }

            Log.i(TAG, "expolit: " + expolit);
        });
    }
}